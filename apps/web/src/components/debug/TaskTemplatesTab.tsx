...
export function TaskTemplatesTab() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-bold">Task Templates ({templates?.length ?? 0})</h3>
      <div className="flex gap-2 items-center">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Template Name *" className="border-2 border-black px-3 py-2 flex-1" />
        <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Category" className="border-2 border-black px-3 py-2 w-32" />
          ...
          <div key={t._id} className="border-2 border-black overflow-hidden">
            <div className="flex items-center justify-between p-3 bg-gray-50 cursor-pointer" onClick={() => setExpandedTemplate(expandedTemplate === t._id ? null : t._id)}>
              <div>
                ...
                {t.isRepeatable && <span className="ml-2 text-xs bg-purple-100 text-purple-700 px-2 py-0.5 border border-purple-300">Repeatable</span>}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-gray-400 font-bold">{expandedTemplate === t._id ? "▼" : "▶"}</span>
                <button onClick={(e) => { e.stopPropagation(); removeTemplate({ id: t._id }); }} className="text-red-500 text-sm font-bold hover:text-red-700">Delete</button>
              </div>
            </div>
            {expandedTemplate === t._id && templateWithFields && (
              <div className="p-3 bg-white border-t-2 border-black space-y-3">
                <div className="text-sm font-bold text-gray-700">Fields:</div>
                ...
                <div className="flex gap-2 items-center">
                  <input value={newFieldLabel} onChange={(e) => setNewFieldLabel(e.target.value)} placeholder="Field Label" className="border-2 border-black px-2 py-1 flex-1 text-sm" />
                  ...
                  <button onClick={handleAddField} className="bg-green-500 text-white px-3 py-1 text-sm font-bold border-2 border-black hover:bg-green-600">Add Field</button>
                </div>
              </div>
            )}
          </div>
