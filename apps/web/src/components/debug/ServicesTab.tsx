...
export function ServicesTab() {
  ...
  const handleAddTemplate = async () => {
    ...
    await addTaskTemplate({ serviceId: expandedService, taskTemplateId: selectedTemplate, order, isRequired: true });
    setSelectedTemplate("");
  };

  return (
    <div className="space-y-4">
      <h3 className="text-lg font-bold">Rutinas ({services?.length ?? 0})</h3>
      <div className="flex gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre de Rutina *" className="border-2 border-black px-3 py-2 flex-1" />
